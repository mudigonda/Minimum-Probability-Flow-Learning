...
description = 'd=78, 10000 samples'

% initialize
d = 78; % number of units
nsamples = 23440; % number of training samples
...
t_samp = tic();
data = load('/media/mudigonda/Gondor/Data/datasets_hippocampus/2bin.mat');
Xall = data.data';
t_samp = toc(t_samp);
...
display(size(Xall))
display('\n The shape of the Coupling matrix are \n')
display(size(Jnew))
